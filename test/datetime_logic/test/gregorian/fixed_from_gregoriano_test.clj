(ns datetime-logic.test.gregorian.fixed-from-gregoriano-test
  (:refer-clojure :exclude [==])
  (:use
    clojure.test
    datetime-logic.test.test-helper
    [datetime-logic.gregorian :only [fixed-from-gregoriano]]
    [clojure.core.logic :exclude [is]])
  (:require
    [clojure.core.logic.fd :as fd]))

(deftest fixed-from-gregoriano-test

  (eg (run* [days]
        (fd/in days (fd/interval 0 Integer/MAX_VALUE))
        (fixed-from-gregoriano 1945 11 12 days))
    => [710347])

  (eg (run* [year]
        (fd/in year (fd/interval 0 Integer/MAX_VALUE))
        (fixed-from-gregoriano year 11 12 710347))
    => [1945])

  (eg (run* [month]
        (fd/in month (fd/interval 0 Integer/MAX_VALUE))
        (fixed-from-gregoriano 1945 month 12 710347))
    => [11])

  (eg (run* [day]
        (fd/in day (fd/interval 0 Integer/MAX_VALUE))
        (fixed-from-gregoriano 1945 11 day 710347))
    => [12])
  ...
  (eg
    (set
      (run* [year days]
        ...
        (fixed-from-gregoriano year 10 31 days)))
